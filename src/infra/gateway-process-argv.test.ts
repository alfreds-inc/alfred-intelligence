...
describe("isGatewayArgv", () => {
  ...
  it("matches product executables but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\openclaw.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/zolven-intelligence", "gateway"])).toBe(true);
