...
describe("slash command browser import", () => {
  ...
  it("keeps provider thinking runtime out of the Control UI import path", async () => {
    ...
    expect(importDeclarations(sharedRegistry)).toEqual([
      ...
      // Fork-local: the shared registry renders the product name into command
      // help text. src/branding.ts is a leaf constants module -- no imports, one
      // exported string -- so it cannot pull provider thinking runtime into the
      // Control UI bundle, which is the boundary this assertion protects.
